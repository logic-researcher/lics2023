...
public class DefinerIntroducer {
	Logger logger = LoggerFactory.getLogger(DefinerIntroducer.class);
	int definer_index = 1;
	...
	public List<Formula> introduceDefiners(AtomicConcept concept, List<Formula> input_list)
		...
		for (Formula formula : input_list) {
			...
			logger.debug("original formla : ");
			logger.debug(formula+" "+concept);
			logger.debug("after introduced :");
			...
			if (subsumee instanceof Exists) {
				...
				if (filler instanceof Exists ||
					...
					if (definer_right_map.get(filler) == null) {
						AtomicConcept definer = new AtomicConcept("Definer" + definer_index);
						definer_index++;
